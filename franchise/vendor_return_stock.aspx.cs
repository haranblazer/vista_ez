...
public partial class franchise_vendor_return_stock : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                {
                    txt_Toid.Text = Session["franchiseid"].ToString();

                    if (Session["franchiseid"].ToString() == "12345")
                    {
        ...
        try
        {
            ...
            if (Cartdt != null)
            {
                ...
                if (element != null)
                {
                    ...
                    SqlCommand com = new SqlCommand("vendor_return_stock", con);
                    ...
                    com.Parameters.AddWithValue("@ToId", HttpContext.Current.Session["franchiseid"].ToString());
    ...
    [WebMethod]
    public static UserDetails GetVendor(string regno)
    {
        UserDetails objUser = new UserDetails();
        try
        {
            SqlConnection con = new SqlConnection(method.str);
            SqlCommand cmd = new SqlCommand("GetVenderDetails", con);
            SqlDataReader dr;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@regno", regno.Trim());
            cmd.Connection = con;
    ...
    {
        SqlParameter[] param = new SqlParameter[] { new SqlParameter("@Userid", HttpContext.Current.Session["franchiseid"].ToString()) }; 
        DataUtility objDu = new DataUtility();
