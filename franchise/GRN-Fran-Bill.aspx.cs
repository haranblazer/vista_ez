...
public partial class franchise_GRN_Fran_Bill : System.Web.UI.Page
{
    ...
    private void userdetail(string srno)
    {
        ...
        DataTable dt = objDUT.GetDataTableSP(sqlparam, "purchaseOrderGstBill");
        if (dt.Rows.Count > 0)
        {
    ...
    private void BindProduct(string srno)
    {
        try
        {
            ...
            DataUtility objDUT = new DataUtility();
            DataSet ds = objDUT.GetDataSetSP(sqlparam, "GetGrN_PrintBill");
    ...
    private void BindVendorDetail(string srno)
    {
        ...
        DataUtility objDUT = new DataUtility();
        DataTable dt = objDUT.GetDataTableSP(sqlparam, "proc_vendorDetail");
        if (dt.Rows.Count > 0)
        {
