...
public partial class secretadmin_returnitem : System.Web.UI.Page
{
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            foreach (GridViewRow gv in GridView1.Rows)
            {
                ...
                HiddenField OFFERID = (HiddenField)gv.FindControl("hdf_OFFERID");
                ...
                if (Convert.ToInt32(OFFERID.Value) > 0)
                {
                    if ((Resale + Damage) != Qty)
                    {
                        lblMsg.Text = "Please enter scheme quantity of this Product " + Code.Text;
                        return;
                    }
                }
                if ((Resale + Damage) > 0)
                ...
                XElement prdXml = new XElement("P",
                     ...
                     new XElement("OFFERID", OFFERID.Value),
            ...
            cmd.Parameters.AddWithValue("@Ret_InvoiceNo", txt_invoiceNO.Text.Trim());
            ...
            cmd.Parameters.AddWithValue("@paymodeValue", ddlpaytype.SelectedValue);
    ...
    private void bindgrid()
    {
        ...
        foreach (DataRow row in dt.Rows)
        {
            ...
            strDtl += "<tr><th>Name</th><th>Qty</th><th>MRP</th><th>CV</th><th>Total</th><th>Damage</th><th>Resale</th></tr>";
    ...
    private void BindForPrint(string InvoiceNo)
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["Returned"].ToString() == "1")
                {
                    btn_Submit.Enabled = false;
                    ddlpaytype.Enabled = false;
                    lblMsg.Text = "Already Returned.";
                }
                ...
                foreach (XElement p in objXml.Elements("P"))
                {
                    ...
                    if (p.Elements("OFFERID").FirstOrDefault().Value.ToString() != "0")
                        row["pname"] = (p.Elements("pname").FirstOrDefault() != null ? p.Elements("pname").FirstOrDefault().Value : string.Empty) + "<span style='color:red;'> Scheme</span>";
                    else
                        row["pname"] = p.Elements("pname").FirstOrDefault() != null ? p.Elements("pname").FirstOrDefault().Value : string.Empty;

                    row["OFFERID"] = p.Elements("OFFERID").FirstOrDefault() != null ? p.Elements("OFFERID").FirstOrDefault().Value : "0";
