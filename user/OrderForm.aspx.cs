...
public partial class d2000admin_BillingFrom : System.Web.UI.Page
{
    ...
    private void BindProducts()
    {
        ...
        try
        {
            ...
            da = new SqlDataAdapter("select ProductName,dpamt as MRP,BVAmt as BV from ShopProductMst where isdeleted=0 order by productId", con);
            da.Fill(dt);
            if (!IsPostBack)
            {
    ...
    private string SaveDetail()
    {
        ...
        if (tempDt != null)
        {
            ...
            if (status == "1")
            {
                utility.MessageBox(this, "Order Send Successfully To Admin");
    ...
    private void BindAddress()
    {
        ...
        try
        {
            com = new SqlCommand("select Appmstaddress1 from appmst where appmstregno=@regno", con);
            //pass session id
            com.Parameters.AddWithValue("@regno", regno);
            con.Open();
    ...
    private void BindGrid()
    {
        ...
        ((Label)GridView1.FooterRow.FindControl("lblTotal")).Text = dt.Compute("sum(total)", "true").ToString().Trim();
    }
