...
public partial class secretadmin_LegwisePV : System.Web.UI.Page
{
   static string fromDate = "", toDate = "", MinV = "750", MaxV = "999999999";
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                BindGrid();
                //txtFromDate.Text = DateTime.Now.Date.AddMonths(-3).ToString("dd/MM/yyyy").Replace("-", "/");
    ...
    public void BindGrid()
    {
        ...
        string fromDate = "", toDate = "", MinV="750", MaxV="999999999";
        ...
        DataTable dt = getdate(fromDate, toDate, ddl_PackType.SelectedValue, MinV, MaxV, Session["userId"].ToString(), "");
        //lbl_Msg.Text = "Total: " + dt.Rows.Count;
        if (dt.Rows.Count > 0)
        { 
            GridView1.Columns[2].FooterText = "Total : " + dt.Rows.Count;
            GridView1.Columns[5].FooterText = dt.Compute("sum(Jan)", "true").ToString();
            GridView1.Columns[6].FooterText = dt.Compute("sum(Feb)", "true").ToString();
            GridView1.Columns[7].FooterText = dt.Compute("sum(Mar)", "true").ToString();
            GridView1.Columns[8].FooterText = dt.Compute("sum(Apr)", "true").ToString();

            GridView1.Columns[9].FooterText = dt.Compute("sum(May)", "true").ToString();
            GridView1.Columns[10].FooterText = dt.Compute("sum(Jun)", "true").ToString();
            GridView1.Columns[11].FooterText = dt.Compute("sum(Jul)", "true").ToString();
            GridView1.Columns[12].FooterText = dt.Compute("sum(Aug)", "true").ToString();

            GridView1.Columns[13].FooterText = dt.Compute("sum(Sep)", "true").ToString();
            GridView1.Columns[14].FooterText = dt.Compute("sum(Oct)", "true").ToString();
            GridView1.Columns[15].FooterText = dt.Compute("sum(Nov)", "true").ToString();
            GridView1.Columns[16].FooterText = dt.Compute("sum(Dec)", "true").ToString();
    ...
    { 
        string PackType= ddl_PackType.SelectedValue, SessionId= Session["userId"].ToString(); 
        //try
        //{
        //    if (txtFromDate.Text.Trim().Length > 0)
        //    {
        ...
        if (e.CommandName == "Jan")
            Response.Redirect("Legdetails.aspx?From=" + secure.Encrypt(fromDate) + "&To=" + secure.Encrypt(toDate) + "&PackType=" + secure.Encrypt(PackType) + "&MinV=" + secure.Encrypt(MinV) + "&MaxV=" + secure.Encrypt(MaxV) + "&userid=" + secure.Encrypt(UserId) + "&MonthName=" + secure.Encrypt(e.CommandName) + "&SessionId=" + secure.Encrypt(SessionId));
    ...
    static private DataTable getdate(string fromDate, string toDate, string PackType, string MinV, string MaxV, string Userid, string MonthName)
    { 
        SqlParameter[] param = new SqlParameter[]
       {
        ...
        DataUtility objDu = new DataUtility();
        return objDu.GetDataTableSP(param, "GetLegwise_RPV");
    }
    ...
    protected void imgbtnExcel_Click(object sender, EventArgs e)
    {
        if (GridView1.Rows.Count > 0)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + DateTime.Now.ToString("dd/MM/yyyy") + "__UserLegwiseRPV.xls");
            Response.Charset = "";
    ...
    protected void imgbtnWord_Click(object sender, EventArgs e)
    {
        if (GridView1.Rows.Count > 0)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + DateTime.Now.ToString("dd/MM/yyyy") + "_LegWisePV.doc");
