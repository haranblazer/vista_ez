...
public partial class secretadmin_AddMonthlyOffer : System.Web.UI.Page
{
    ...
    public void BindTour()
    {
        ...
        FranType=(Case when FranType=5 then 'Top Point' else '' End), 
        BillType=(Case when BillType=1 then 'Topper' when BillType=3 then 'Generation' else '' End) ,IsActive 
        from MonthlyIncome_Slab order by MId desc";
        DataUtility objDu = new DataUtility();
