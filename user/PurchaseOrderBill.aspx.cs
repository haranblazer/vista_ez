...
public partial class franchise_PurchaseOrderBill : System.Web.UI.Page
{
            ...
            if (Request.QueryString["id"] != null)
            {
               ...
               // status = Convert.ToInt32(Request.QueryString["st"].ToString());
                ...
                //{
                    if (status == 1)
                    {
                        BindForPrint(objUtil.base64Decode(Request.QueryString["id"].ToString()));
                        BindCompanyDetail(int.Parse(objUtil.base64Decode(Request.QueryString["id"].ToString())));
                        bill();
                    }
    ...
    public void bill()
    {
        ...
        com.Parameters.AddWithValue("@srno", Convert.ToInt32(objUtil.base64Decode(Request.QueryString["id"])));
    ...
    public void BindCompanyDetail(int orderNo)
    {
        ...
        try
        {
            ...
            con = new SqlConnection(method.str);
            SqlDataAdapter adapter = new SqlDataAdapter("ordercompanydetail", con);
    ...
    private void BindForPrint(string billId)
    {
        try
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("orderprintbill", con);
            adapter.SelectCommand.CommandType = CommandType.StoredProcedure;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTotalPy.Text = lblAmt.Text = txtNetAmt.Text = Math.Round(double.Parse(dt.Rows[0]["total"].ToString())).ToString("#0.00");
