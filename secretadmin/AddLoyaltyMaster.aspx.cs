...
public partial class secretadmin_AddLoyaltyMaster : System.Web.UI.Page
{
    ...
    {
        String Qry = @"Select BV_Point, LoyalityType, Max_BV_Point=isnull(Max_BV_Point,0), LoyaltyName, Commission, MaxRoyalty, RoyaltyType=isnull(RoyaltyType,0),
            StartDate = convert(nvarchar, StartDate, 103), EndDate = convert(nvarchar, EndDate, 103), IsActive
            from LoyaltyMonthlyIncome_Slab t where MId=@MId";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            txt_Max_BV_Point.Text = dt.Rows[0]["Max_BV_Point"].ToString();
            txt_LoyaltyName.Text = dt.Rows[0]["LoyaltyName"].ToString();

            ddl_LoyalityType.SelectedValue = dt.Rows[0]["LoyalityType"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            String Max_BV_Point = txt_Max_BV_Point.Text == "" ? "0" : txt_Max_BV_Point.Text;
            ...
            SqlCommand cmd = new SqlCommand("[AddEditLoyaltyOffer]", con);
            ...
            cmd.Parameters.AddWithValue("@Max_BV_Point", Max_BV_Point);
            cmd.Parameters.AddWithValue("@LoyaltyName", txt_LoyaltyName.Text.Trim());
            cmd.Parameters.AddWithValue("@LoyalityType", ddl_LoyalityType.SelectedValue);
            ...
            if (flag == "0")
            {
                Response.Redirect("AddRoyality.aspx");
            }
            else
            {
