...
public partial class secretadmin_LoyalityPayoutList : System.Web.UI.Page
{ 
    ...
    {
        DataUtility objDu = new DataUtility();
        DataTable dt = objDu.GetDataTable("Select payoutno, Dates=Cast(DateName(Month, paytodate) as nvarchar(3)) + '-' + RIGHT(DateName(Year, paytodate),2) from Loyaltydate order by payoutno desc");
        if (dt.Rows.Count > 0)
        {
    ...
    public static SponsorDetails[] GetLoyalityData(string Payoutno, string Regno)
    {
        ...
        SqlParameter[] param = new SqlParameter[]
       {
        ...
        DataUtility objDu = new DataUtility();
        DataTable dt = objDu.GetDataTableSP(param, "GetLoyalityDetails");
