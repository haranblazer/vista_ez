...
public partial class secretadmin_PrimarySale : System.Web.UI.Page
{
    ...
    private void Binddata()
    {
        ...
        DataTable dt = obj_du.GetDataTableSP(param, "Get_primary_dashboard");
        // Session["dt_2"] = dt;
        if (dt.Rows.Count > 0)
        {
            if (dt.Rows[0]["RoleId"].ToString() == "1")
            {
                ...
                lbl_INVWISE.InnerText = dt.Rows[0]["TotalPrimarySales"].ToString();
                lbl_DEPOWISE.InnerText = dt.Rows[0]["TotalPrimarySales"].ToString();
                lbl_BUYERWISE.InnerText = dt.Rows[0]["TotalPrimarySales"].ToString();
            }
