...
public partial class secretadmin_GenTourList : System.Web.UI.Page
{
    ...
    public static UserDetails[] BindTable(string min, string max, string Userid, string Tid, string RankId)
    {
        ...
        try
        {
            ...
            while (dr.Read())
            {
                ...
                data.TourTypeValue = dr["TourTypeValue"].ToString();
    ...
    public class UserDetails
    {
        ...
        public string TourTypeValue { get; set; }
    ...
    {
        DataUtility objDu = new DataUtility();
        DataTable dt = objDu.GetDataTable("Select Tid, TourName=TourName +' '+ Convert(nvarchar(10), StartDate, 103)+' To '+ Convert(nvarchar(10), ExpDate, 103)+' (REQD: '+Cast(Cast(Condition as int) as nvarchar(10))+') ' from Tourmst Where Pack_Rep=3");
        if (dt.Rows.Count > 0)
        {
