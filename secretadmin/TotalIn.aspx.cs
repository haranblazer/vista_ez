        ...
        if (!Page.IsPostBack)
        {
            if (Request.QueryString.Count > 0)
            {
                ...
                bindgrid(Convert.ToInt32(objUtil.base64Decode(Request.QueryString["pid"].ToString()))
                    , Convert.ToInt32(objUtil.base64Decode(Request.QueryString["typ"].ToString()))
                    , objUtil.base64Decode(Request.QueryString["u"].ToString()));
            }
        }
    }
    private void bindgrid(int pid, int typ, string Userid)
    {
        con = new SqlConnection(method.str);
        da = new SqlDataAdapter("StockInhandDetail", con);
        da.SelectCommand.CommandType = CommandType.StoredProcedure;
        da.SelectCommand.Parameters.AddWithValue("@regno", Userid);
