...
public partial class secretadmin_ProductWisePrimarySales : Page
{
    ...
    public static UserDetails[] BindTable(string min, string max)
    {
        ...
        try
        {
            ...
            SqlDataReader dr = objDu.GetDataReaderSP(param, "GetPrimarySaleProductWise");
            while (dr.Read())
            {
                UserDetails data = new UserDetails();
                data.ProductCode = dr["ProductCode"].ToString();
                data.ProductName = dr["ProductName"].ToString();
                data.PurchaseQty = dr["PurchaseQty"].ToString();
    ...
    //    };
    //    DataUtility objDu = new DataUtility();
    //    DataTable dt = objDu.GetDataTableSP(param, "GetPrimarySaleProductWise");
    //    if (dt.Rows.Count > 0)
    //    {
    ...
    //        GridView1.Columns[8].FooterText = dt.Compute("sum(SalesQty)", "true").ToString();
    //        GridView1.Columns[10].FooterText = dt.Compute("sum(SalesValue)", "true").ToString();
    ...
    //        Response.AddHeader("content-disposition", "attachment;filename=" + DateTime.Now.ToString("dd/MM/yyyy") + "_ProductWisePrimarySale.xls");
    //        Response.Charset = "";
