...
public partial class secretadmin_LegwisePV : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ddl_PackType.Items.Insert(0, new ListItem(method.GBV, "3"));
    ...
    public void show()
    {
        ...
        DataTable dt = getdate(fromDate, toDate, ddl_PackType.SelectedValue, MinV, MaxV, txt_Userid.Text.Trim(), "");
    ...
    {
        string fromDate = "", toDate = "", PackType = ddl_PackType.SelectedValue, SessionId = txt_Userid.Text.Trim();
        try
        {
            if (txtFromDate.Text.Trim().Length > 0)
            {
    ...
    public static SponsorDetails[] GetMonthWiseData(string FromDate, string ToDate, string PackType, string MinV, string MaxV, string userid, string MonthName)
    {
        ...
        DataTable dt = getdate(fromDate, toDate, PackType, MinV, MaxV, userid, MonthName);
        foreach (DataRow dr in dt.Rows)
        {
            ...
            data.Mobile = dr["MobileNo"].ToString();
