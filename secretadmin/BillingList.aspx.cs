...
public partial class user_OrderList : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        try
        {
            ...
            con = new SqlConnection(method.str);
            //SqlDataAdapter ad = new SqlDataAdapter("billsearch", con);
            SqlDataAdapter ad = new SqlDataAdapter("FranchiseBill", con);
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.Equals("cancel"))
        {
           ...
           // userid= GridView1.DataKeys[row.RowIndex].Values[1].ToString();
           ...
           monthname = GridView1.DataKeys[row.RowIndex].Values[1].ToString();
            ...
            try
            {
                ...
                if (DateTime.Now.ToString("MMMM") == monthname)
                {
                 ...
                 //userid = GridView1.DataKeys[row.RowIndex].Values[4].ToString();
                 invoiceno= GridView1.DataKeys[row.RowIndex].Values[0].ToString();
                 //status = Convert.ToInt32(GridView1.DataKeys[row.RowIndex].Values[3].ToString());
                 con = new SqlConnection(method.str);
                 ...
                 con.Open();
                 cmd.ExecuteNonQuery();
                 status2 = cmd.Parameters["@flag"].Value.ToString();
                 if (status2 == "1")
                 {
                 ...
                 con.Close();
                    ...
                    //string a = GridView1.Rows[Convert.ToInt32(e.CommandArgument)].Cells[1].Text;
                    // b = GridView1.Rows[Convert.ToInt32(e.CommandArgument)].Cells[0].Text;
                    // gvActivePinList.DataKeys[e.NewEditIndex].Values[0].ToString()
                    //con = new SqlConnection(method.str);
                    //SqlCommand cmd = new SqlCommand("select Detail,totalbv,salesrep from billmst where srno='" + b + "'", con);
                    //con.Open();
                    //SqlDataReader dr;
                    ...
                    //    pbv = int.Parse(dr["totalbv"].ToString());
                    //    salesrep = dr["salesrep"].ToString();
    ...
    //        {
    //            string qty = xn["Qnt"].InnerText;
    //            string pid = xn["cd"].InnerText;
    //            objDUT = new DataUtility();

    //            if (salesrep == "admin")
    //            {

    //                objDUT.ExecuteSql("update ShopProductMst Set QTY=QTY+" + qty + " where Productid='" + pid + "'");
    ...
    //                objDUT.ExecuteSql("update franchisestock Set QTY=QTY+" + qty + " where Productid='" + pid + "'");
    //            }
    ...
    //        con = new SqlConnection(method.str);
    //        com = new SqlCommand("cancelbill", con);
    //        com.CommandType = CommandType.StoredProcedure;
    //        com.Parameters.AddWithValue("@appmstid", a);
    //        com.Parameters.AddWithValue("@pbv", pbv);
    //        com.Parameters.AddWithValue("@invoiceno",b);
    //        com.Parameters.Add("@flag", SqlDbType.VarChar,50).Direction = ParameterDirection.Output;
    //        con.Open();
    //        com.ExecuteNonQuery();
    //        con.Close();

    //        //objDUT.ExecuteSql("a +update appmst set pbvamt=pbvamt - '" + pbv + "' where AppMstID='" +  "'");

    //        //objDUT.ExecuteSql("update AppMst set gbvAmt=gbvAmt - '" + pbv + "' where AppMstID in (select ParentID from apptransponsor where AppMstID='" + a + "'");
    ...
    //            "CONVERT(varchar,Doe,103) AS doe from BillMst o inner join appmst a on a.appmstid=o.appmstid and o.status=1 and o.SalesRepId='" + DropDownList1.SelectedValue + "'" +
    //            "and (a.AppMstRegNo like @search or  a.AppMstFName like @search or o.Amount  like @search or o.srno like @search) and " +
    //            "(case when @min is null or len(@min)=0 then 1 when @min is not null and @min<>'' and  cast(floor(cast(o.Doe as float)) as datetime)>=cast(floor(cast(convert(datetime,@min) as float)) as datetime) then 1 " +
    //            "else 0 end)=1 " +
    //            "and  " +
