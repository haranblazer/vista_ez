...
public partial class secretadmin_blog_AllBlogs : System.Web.UI.Page
{
    ...
    public void BindBlogDetails()
    {
        try
        {
            cmd = new SqlCommand("Select * from Tb_BlogTags", con);
    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("AddEditBlogTags",con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@tagid", hiddenID.Value);
    ...
    protected void AddEdit_Click(object sender, EventArgs e)
    {
        ...
        if (btn == null)
        {
            using (GridViewRow row = (GridViewRow)((LinkButton)sender).Parent.Parent)
            {
                ...
                id = row.Cells[1].Text;
                hiddenID.Value = id;
                txttagname.Text = row.Cells[2].Text;
