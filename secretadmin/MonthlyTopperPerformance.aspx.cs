...
public partial class secretadmin_MonthlyTopperPerformance : System.Web.UI.Page
{
        ...
        try
        {
            SqlParameter[] param = new SqlParameter[]
           {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                GridView1.Columns[7].FooterText = (string.IsNullOrEmpty(dt.Compute("sum([TCC Matching])", "true").ToString()) ? "0.00" : dt.Compute("sum([TCC Matching])", "true").ToString());
                GridView1.Columns[8].FooterText = (string.IsNullOrEmpty(dt.Compute("sum([Topper Matching Income])", "true").ToString()) ? "0.00" : dt.Compute("sum([Topper Matching Income])", "true").ToString());
                GridView1.Columns[9].FooterText = (string.IsNullOrEmpty(dt.Compute("sum([Topper Reward Income])", "true").ToString()) ? "0.00" : dt.Compute("sum([Topper Reward Income])", "true").ToString());
                GridView1.Columns[10].FooterText = (string.IsNullOrEmpty(dt.Compute("sum([Topper Fund Income])", "true").ToString()) ? "0.00" : dt.Compute("sum([Topper Fund Income])", "true").ToString());
                GridView1.Columns[11].FooterText = (string.IsNullOrEmpty(dt.Compute("sum([Gross Payout])", "true").ToString()) ? "0.00" : dt.Compute("sum([Gross Payout])", "true").ToString());
    ...
    private void Bind_Month()
    {
        ...
        from PayoutDate ) t order by t.Dates desc");
        if (dt.Rows.Count > 0)
        {
