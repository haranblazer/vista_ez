<%@ Page Title="" Language="C#" MasterPageFile="MasterPage.master" AutoEventWireup="true" CodeFile="Admin_GST_Turnover.aspx.cs" Inherits="secretadmin_Admin_GST_Turnover" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">

    <h4 class="fs-20 font-w600  me-auto float-left mb-0">Admin GST Turnover Report (Rs 10 Lacs and above)</h4>
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <asp:DropDownList ID="ddl_Asso_Fran" runat="server" CssClass="form-control">
<%--                <asp:ListItem Value="ASSOCIATE"><%=method.Associate%></asp:ListItem>--%>
        ...
        <div class="col-md-2">
            <asp:TextBox ID="txt_amt" runat="server" placeholder="Enter Amount" Text="1000000"
                MaxLength="15" CssClass="form-control" required="required"></asp:TextBox>
        </div>
    ...
    <script type="text/javascript">
        var pageUrl = '<%=ResolveUrl("Admin_GST_Turnover.aspx")%>';


        function BindTable() {
            let min = dateFormate($('#<%=txtFromDate.ClientID%>').val()),
                max = dateFormate($('#<%=txtToDate.ClientID%>').val()),
                Asso_Fran = $('#<%=ddl_Asso_Fran.ClientID%>').val(),
                ConditionAmt = $('#<%=txt_amt.ClientID%>').val();

            $('#LoaderImg').show();
            $.ajax({
                type: "POST",
                url: pageUrl + '/BindTable',
                data: '{ min: "' + min + '", max: "' + max + '", Asso_Fran: "' + Asso_Fran + '", ConditionAmt: "' + ConditionAmt + '"}',
