...
public partial class Common_ReturnInv : System.Web.UI.Page
{
    ...
    private void BindCompany(string srno)
    {
        ...
        DataTable dt = objDUT.GetDataTableSP(sqlparam, "CompanydetailReturn");
        if (dt.Rows.Count > 0)
        {
    ...
    private void userdetail(string srno)
    {
        ...
        DataUtility objDUT = new DataUtility();
        DataTable dt = objDUT.GetDataTableSP(sqlparam, "Printbill_Return");
        if (dt.Rows.Count > 0)
        {
           ...
           // lbl_CourierCharges.Text = dt.Rows[0]["DelCharge"].ToString();
            ...
            if (!string.IsNullOrEmpty(SalesGST))
            {
                lbl_NetTotal.Text = double.Parse(dt.Rows[0]["NetAmt"].ToString()).ToString("#0.00");
            }
            else
            {
                lbl_GrossTotal.Text = double.Parse(dt.Rows[0]["NetAmt"].ToString()).ToString("#0.00");
                lbl_NetTotal.Text = double.Parse(dt.Rows[0]["NetAmt"].ToString()).ToString("#0.00");
            }
    ...
    private void BindProduct(string srno)
    {
        try
        {
            ...
            DataUtility objDUT = new DataUtility();
            DataSet ds = objDUT.GetDataSetSP(sqlparam, "GetReturnProduct");
